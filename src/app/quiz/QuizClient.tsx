    ...
    >
      ...
      <AnimatePresence>
        {showExitConfirm && (
          ...
          >
            <motion.div
              key="exit-dialog-card"
              initial={{ scale: 0.92, opacity: 0, y: 14 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.92, opacity: 0, y: 14 }}
              transition={{ type: "spring", stiffness: 350, damping: 28 }}
              onClick={(e) => e.stopPropagation()}
              className="w-full max-w-[380px] bg-white rounded-[28px] p-6 sm:p-7 flex flex-col gap-5 relative shadow-2xl border border-[#F5DADA] cursor-default"
            >
              {/* Top-Right Close Button (X) */}
              <button
                type="button"
                onClick={() => setShowExitConfirm(false)}
                className="absolute top-4 right-4 w-8 h-8 rounded-full bg-[#FFF0F0] text-[#9A7878] hover:text-[#D94040] hover:bg-[#FADADD] transition-colors flex items-center justify-center cursor-pointer"
                title="Close"
              >
                <PiX size={16} />
              </button>
              ...
              >
                <PiWarningCircleFill size={26} style={{ color: "#F07070" }} />
              </div>
              ...
              <div className="flex flex-col gap-2.5 pt-1">
                ...
                <button
                  type="button"
                  onClick={() => setShowExitConfirm(false)}
                  className="flex items-center justify-center rounded-full font-semibold transition-colors duration-200 cursor-pointer text-[#4A3333] border border-[#F5DADA] hover:border-[#E8BFBF] bg-white text-[14px]"
                  style={{ minHeight: "46px" }}
                >
                  {done ? "Stay on checkout" : "Keep going"}
                </button>
              </div>
            </motion.div>
          </motion.div>
      ...
      {/* Header */}
      <header className={`relative z-10 w-full px-5 py-5 flex items-center justify-between ${done ? "max-w-6xl" : "max-w-[600px]"} mx-auto transition-all duration-300`}>
        <button
          type="button"
          onClick={() => setShowExitConfirm(true)}
          aria-label="by tearsize home"
